import { TopDayHourDTO, TopHoursReportDTO } from "@/modules/users/application/dtos/reports/topHourDTO";
...
export class topHourDayUseCase {
  ...
  async execute(company_id: string, date_from?: string, date_to?: string): Promise<TopHoursReportDTO> {
    ...
    if (company.subscription_plan.name !== "Premium") {
      throw new ValidationReportsError("Esta funcion no esta disponible en tu plan");
    }
    ...
    return {
      date_from: topHourDay.date_from,
      date_to: topHourDay.date_to,
